...
{% hint style="info" %}
System limits for Snyk Secrets in the SCM:
...
Snyk Secrets detects hard-coded secrets, credentials, and API keys in your source code management (SCM) repositories before they are exposed. Snyk includes secret detection during routine pull request checks and manual SCM imports.

After you enable Snyk Secrets, Snyk automatically scans pre-existing Projects only when it receives a push event from the SCM. You can also trigger a scan by re-importing the Project.
...
The following example excludes the `vendor` directory from all supported Snyk products, and excludes the PEM test fixtures and the `examples` directory from Snyk Secrets only:
...
For the full exclusion pattern syntax and formatting rules, visit [Exclusion syntax of the `.snyk` file](https://docs.snyk.io/scan-fix-and-prevent/scan-with-snyk/snyk-projects/import-project-repository/exclude-directories-and-files-from-project-import#exclusion-syntax-of-the-.snyk-file).
...
To exclude paths when you scan locally with the Snyk CLI, visit [Exclude files and directories from a scan](../../snyk-cli/scan-and-maintain-projects-using-the-cli/snyk-cli-for-secrets/secrets-scanning-in-the-snyk-cli.md#exclude-files-and-directories-from-a-scan).
