...
* Internet-accessible repositories.\
  If your repositories are not internet-accessible, you must use [Snyk Broker](https://docs.snyk.io/platform-administration/snyk-broker/snyk-broker). This requires creating a startup script. For the script and instructions, visit [GitHub Enterprise - install and configure using Docker](https://docs.snyk.io/platform-administration/snyk-broker/classic-broker/install-and-configure-snyk-broker/github-enterprise-prerequisites-and-steps-to-install-and-configure-broker/github-enterprise-install-and-configure-using-docker).
* A public or private GitHub project.
* A Snyk Organization Admin user role. To learn more, visit [Pre-defined roles](https://docs.snyk.io/platform-administration/user-management/pre-defined-roles#organization-level-permissions).
...
2. Scroll to **Snyk PR status checks**; see [Configure PR Checks](https://docs.snyk.io/scan-fix-and-prevent/prevent/pull-request-checks/configure-pull-request-checks) for details.
...
4. Broker Token (`mandatory`): Create and add your Broker token if you use Snyk Broker.
   * Generate your Broker token by following the instructions from the [Obtain your Broker token for Snyk Broker](https://docs.snyk.io/platform-administration/snyk-broker/classic-broker/prepare-snyk-broker-for-deployment/obtain-the-tokens-required-to-set-up-snyk-broker) page.
