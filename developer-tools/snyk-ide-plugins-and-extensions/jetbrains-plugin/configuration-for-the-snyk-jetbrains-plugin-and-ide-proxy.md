...
* **Custom endpoint**: Specify the Snyk API endpoint for a custom multi-tenant or single-tenant setup. If you are using `https://api.snyk.io`, no configuration is required. For details, see the list of [IDE URLs](https://docs.snyk.io/snyk-data-and-governance/regional-hosting-and-data-residency#ides-urls).\
  Snyk automatically redirects multi-tenant users outside the `SNYK-US-01` region to the domain of their authenticated email. Snyk does not redirect users who use a custom URL, such as those in single-tenant setups.
...
These options allow you to customize how the plugin dependency is handled by the Snyk CLI.
...
* **Path to Snyk CLI**: Allow changing the Snyk CLI file path.
...
**Scan automatically on start-up and save**: Enable automatic scans when saving files and when opening a Project.
