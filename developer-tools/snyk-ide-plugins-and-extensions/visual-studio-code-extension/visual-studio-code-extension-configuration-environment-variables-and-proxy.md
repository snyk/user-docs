...
* **Custom Endpoint**: Specify the Snyk API endpoint for a custom multi-tenant or single-tenant setup. If you are using `https://api.snyk.io`, no configuration is required. For details, see the list of [IDE URLs](https://app.gitbook.com/s/ELvljsaLKPkSpffOkmsQ/regional-hosting-and-data-residency#ides-urls).\
  Snyk automatically redirects multi-tenant users outside the `SNYK-US-01` region to the correct domain based on their authenticated email. Snyk does not redirect users if they must use a custom URL, for example, in single-tenant setups.
...
**Trusted Folders**: Link to the `settings.json` file that lists the folders marked as trusted. Use this setting only in advanced cases or when certain folders should be marked as not trusted.
...
* On macOS, the process `launchd` must know the environment variables to launch the IDE directly from the Finder. You can set environment variables for applications launched using the Finder by using the `launchctl setenv` command, for example, on start-up or through a script you launch at user login.
