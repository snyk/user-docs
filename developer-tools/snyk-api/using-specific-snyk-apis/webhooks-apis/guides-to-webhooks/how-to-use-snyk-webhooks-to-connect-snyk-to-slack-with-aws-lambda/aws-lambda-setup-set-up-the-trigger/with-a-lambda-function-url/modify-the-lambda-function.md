---
description: How to modify the AWS Lambda function for the Snyk to Slack integration
nav_context: classic
---

{% include "../../../../../../../.gitbook/includes/new-navigation-banner.md" %}

# Modify the Lambda function

1. Open your Lambda function and click on **Configuration**.
2.  Select **Function URL** and then **Create function URL**.

    <figure><img src="../../../../../../../.gitbook/assets/modify-lambda-function.png" alt="Lambda function configuration in the AWS console"><figcaption></figcaption></figure>
3.  Change **Auth type** to **NONE** and save.

    <figure><img src="../../../../../../../.gitbook/assets/modify-lambda-function-2.png" alt="Setting the function URL Auth type to NONE"><figcaption></figcaption></figure>
4. Copy the function URL to use in the next step of the guide.
5.  The function URL is now ready for use and a resource-based policy has been added that you can verify under **Permissions**.

    <figure><img src="../../../../../../../.gitbook/assets/modify-lambda-function-3.png" alt="Copying the Lambda function URL"><figcaption></figcaption></figure>
