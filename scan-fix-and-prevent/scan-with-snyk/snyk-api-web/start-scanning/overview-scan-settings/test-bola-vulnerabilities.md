...
2. When configuring authentication, select the **Add additional user for authorization testing** check box.
3. Configure the authentication for the second user using the same method (authentication payload or static headers/cookies). To reduce false positives, the second user must have the same level of privileges as the first user, or lower.
