...
To delete the template, use the endpoint [Delete pull request template for group](https://docs.snyk.io/developer-tools/snyk-api/reference/pull-request-templates#delete-groups-group_id-settings-pull_request_template).
...
If you use [Snyk Broker](https://docs.snyk.io/platform-administration/snyk-broker/snyk-broker), you must use a Broker at version 4.188.0 or higher and enable the Broker to fetch the custom PR templates using the `ACCEPT_CUSTOM_PR_TEMPLATES` environment variable.
