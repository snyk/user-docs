...
For configuring 1,2 and 3 please follow the example in [Configure Postman Collection targets](../api-targets/configure-postman-collection-targets.md).
...
1. Add the authentication request to the `auth` folder and validate that the test passed.
    ...
    <figure><img src="../../../.gitbook/assets/configure-postman-authentication-folder-auth.png" alt="Postman collection showing authentication folder, and the result of the test."></figure>
...
2. Add the check token request to the `logout-detection` folder. Then navigate to the **Scripts** tab of the request and add the following test in the **Post-response** to validate that your token is still valid:
    ...
    <figure><img src="../../../.gitbook/assets/configure-postman-authentication-folder-logout.png" alt="Postman collection showing logout detection folder, and the result of the test."></figure>
...
3. Configure the authentication variables:
    ...
    1. **VARIABLE NAME**: Select the name of the variable as defined in your test script (for example, `bearerToken`).

    1. **PLACE VARIABLE CONTENT IN**: Select where to send the variable content - **Header** or **Cookie**.

    1. **FIELD NAME**: Enter the name of the header or cookie field (for example, `Authorization`).

    1. **VALUE PREFIX**: Enter an optional prefix added before the variable value (for example, `Bearer`).
...
1. Select the folder from the schema file that contains the logout request. For our example scenario, you should select the `logout-detection` folder.
