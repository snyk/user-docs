...
For more information about coverage reports, visit [Coverage report](../../review-and-fix/reports/coverage-report.md) and [Generate CSV coverage report](../../review-and-fix/reports/generate-csv-coverage-report.md).
...
1. Click the target name to see its details.
1. Click **Scan Activity** to see the list of scans.
1. Click **View** for the scan you want to check.
1. Click **Crawling Report** to download the spreadsheet with scanned URLs.
1. Check if URLs that are only available for authenticated users appear in the report.
...
| Cause | Solution |
|-------|----------|
| The target is a SPA with its backing API on a different URL. | Add the API URL as an extra host in target settings. Visit [Configure extra hosts](../../configure-targets/web-targets/configure-extra-hosts.md). |
...
1. Click **View** for the scan you want to check.
1. Click **Crawling Report** to download the spreadsheet with scanned URLs.
1. Check if URLs started receiving HTTP 403 error status at some point during the scan.
1. Open a browser in incognito mode and test those URLs to see if a WAF is blocking access.

If a WAF blocks access to URLs, Snyk API & Web cannot scan them.

| Cause | Solution |
|-------|----------|
| A WAF is blocking access to URLs during the scan. | Add Snyk API & Web IPs to the WAF's allowlist. Visit [Configure IPs in WAFs](../../start-scanning/scan-access-and-connectivity/configure-ips-in-wafs.md). |
...
1. Click **View** for the scan you want to check.
1. Click **Crawling Report** to download the spreadsheet with scanned URLs.
1. Check if URLs have HTTP 403 error status.
1. Open a browser in incognito mode and test those URLs to see if a WordPress plugin is blocking access.

If a WordPress plugin is blocking access to URLs, Snyk API & Web cannot scan them.

| Cause | Solution |
|-------|----------|
| A WordPress plugin (for example, WordFence) is blocking access to URLs. | Configure the WordPress plugin to allow requests from Snyk API & Web IPs. Visit [Scanner IP address](../../start-scanning/scan-access-and-connectivity/scanner-ip-address.md) for the scanner's outgoing IP address. |
