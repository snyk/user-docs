...
1. Open the **Authentication** tab and review the **LOGIN FORM** configuration.
1. Note the login URL and credentials.
1. Open a browser and navigate to the target's login URL.
1. Attempt to log in with the current credentials.

If the login fails, check the following causes and solutions:

| Cause | Solution |
|-------|----------|
| The credentials are invalid. | Obtain valid login credentials and update them in target settings. Visit [Login form](../../configure-targets/authentication/configure-login-form.md). |
| The credentials expired. | Obtain new login credentials and update them in target settings. Visit [Login form](../../configure-targets/authentication/configure-login-form.md). |
...
1. Open a browser and navigate to the target's login URL.
1. Check if the login page displays a standard login form.

If the login is not a login form, the target authentication fails.

| Cause | Solution |
|-------|----------|
| The login flow is not a login form but a complex login (for example, multi-step login). | Configure the target authentication to use a login sequence, which supports complex logins. Visit [Login sequence](../../configure-targets/authentication/configure-login-sequence.md). |
...
1. Navigate to the authentication page with the login form.
1. Check if a WAF blocks access.

If a WAF blocks access to the authentication page, Snyk API & Web cannot authenticate.

| Cause | Solution |
|-------|----------|
| A WAF is blocking access to the authentication page with the login form. | Add Snyk API & Web IPs to the WAF's allowlist. Visit [Configure IPs in WAFs](../../start-scanning/scan-access-and-connectivity/configure-ips-in-wafs.md). |
