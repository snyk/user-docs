...
This document outlines what constitutes a test, so the customer can understand usage against the subscription allocation.

There are two main types of tests:

* Recurring: The Snyk application triggers these tests, based on the customer’s configurations, and they occur at a set cadence (for example, daily or weekly). The Web UI, CLI, or API triggers these tests, and a cron job, typically in the SCM, implements them.\
  \
  Note that if you have chosen the weekly retesting interval, and a manual test occurs before one week ends, the next test occurs a week after the manual test. Re-tests all begin at a set time each day. These re-tests can take sufficient time to process, such that a particular test takes place on the eighth day after the last test. If you see a test interval that is longer than you expect and you are concerned, contact Snyk support.
* Manual: The customer triggers these tests through a specific action. These tests can occur at any cadence in the available functionality of the capability. You can trigger these tests in a number of different ways, including:
  * API - triggered by API call
  * CLI - triggered by CLI commands
  * IDE - triggered by save or autosave (can vary by IDE)
...
Each customer’s specific usage and configurations are distinct, so Snyk uses the criteria described here to determine what constitutes a test.
...
* Snyk Container: the number of dockerfiles or other container files and container images scanned by the customer in the application.
...
* Team and Enterprise Snyk Plans: test limits apply only to Snyk Code and Snyk Open Source. Tests executed through the Snyk IDE plugin do not count towards test limits.
