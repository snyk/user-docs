...
For details, see [Git repositories (SCMs) integrations with Snyk](https://docs.snyk.io/developer-tools/integrations/scm-integrations/organization-level-integrations).
...
* At scale, Snyk recommends using the API. APIs are available with the Snyk Enterprise plan.
  * Use the [Snyk API](https://docs.snyk.io/developer-tools/snyk-api/snyk-api) to import your Projects. This leverages an existing source control integration and can be used to automate processes.
  * The [API-import](https://docs.snyk.io/developer-tools/snyk-apps/tool-snyk-api-import) tool uses the API to manage onboarding at scale for large enterprises and is the suggested tool to use at scale. The source control structure will need to be mirrored.
...
1. [Install the CLI](https://docs.snyk.io/developer-tools/snyk-cli/install-the-snyk-cli) using one of the appropriate methods as part of the build script.
2. [Authenticate to use the CLI](https://docs.snyk.io/developer-tools/snyk-cli/snyk-cli/authenticate-to-use-the-cli) by using the `snyk auth` command or an environment variable.
