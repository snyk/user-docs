# Configure Asset Management with Snyk Essentials and Snyk AppRisk

{% hint style="info" %}
Snyk Essentials is included in Snyk Enterprise. Snyk AppRisk is an additional purchase.
{% endhint %}

## Prerequisites for Snyk Essentials and Snyk AppRisk
...
## Access Snyk Essentials and Snyk AppRisk
...
* Access a Group that has Snyk Essentials or Snyk AppRisk enabled.
...
{% hint style="warning" %}
**Integration Hub** is a distinct integration interface dedicated to asset management. It is available with Snyk Essentials and Snyk AppRisk, separate from the Organization integrations interface which is used for security and license scanning.
{% endhint %}

The integrations at the Organizational level that enable Snyk Code, Snyk Open Source, Snyk Container, and Snyk IaC scanning are generally maintained or restricted by development. When you set a token on the Group Level Integrations screen, it is primarily intended for security purposes, allowing for a comprehensive view of access. This token grants broad access to all repositories, even those that individual developers may not have permission to access. It helps ensure that the security perspective on assets aligns with the views of the development teams.
...
\
With regards to Group level integrations for Snyk Essentials and Snyk AppRisk, the purpose of a token is to provide an overview of the current assets, regardless of what is imported into Snyk for security testing/monitoring.
...
The Snyk Essentials and Snyk AppRisk functionality is split across several menu options from the Group level.&#x20;
...
If you are using Snyk Essentials or Snyk AppRisk for the first time, Snyk recommends you to first use the Coverage filter to determine where you have Snyk implemented. Then, you can use the Coverage Gap filter to identify the assets that do not meet the coverage requirements set in a **Set coverage control** policy.
