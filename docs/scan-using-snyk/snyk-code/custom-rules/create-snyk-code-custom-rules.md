...
You can save custom rules from useful queries and add metadata. The metadata is used to create YAML formatted code. Export the YAML content as a rule in the [.snyk file](../../prioritize-your-issues/the-.snyk-file.md) of the relevant repository to run it alongside Snyk's in-house rules in future scans.
...
5. Configure the metadata:
   ...
   * **Severity**: Set the severity of the result based on [Snyk security levels](../../prioritize-your-issues/severity-levels.md#introduction-to-snyk-severity-levels). Snyk Code uses only three severity levels: `Low`, `Medium` and `High.`
...
7. Export the query as a rule by copying the YAML content to the [.snyk file](../../prioritize-your-issues/the-.snyk-file.md) in the relevant repository.&#x20;
