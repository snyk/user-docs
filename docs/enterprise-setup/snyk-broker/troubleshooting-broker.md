...
| `broker-client-url-validation` | <p>Broker Client Configuration Check validates the <code>BROKER_CLIENT_URL</code> value to be as validated as possible. It verifies it contains a scheme (<code>http or https</code>) and if <code>https</code>, an SSL certificate+key is loaded or is possibly TLS terminated upstream.<br><br>If using TLS termination and you do not require a certificate+key in the Broker Client, add the environment variable <code>BROKER_CLIENT_URL_TLS_TERMINATED</code> to signal TLS termination in the preflight check.</p> | N/A                                                                                        |
