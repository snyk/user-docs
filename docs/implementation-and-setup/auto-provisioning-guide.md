...
1. **Single Sign-On** - use your company's existing identity management system, see [Single Sign-On (SSO) for authentication to Snyk](../snyk-platform-administration/single-sign-on-sso-for-authentication-to-snyk/) for more details.
2. **Third-party authentication** - Snyk supports a list of third-party identity providers, see [Authentication for third-party tools](../developer-tools/authentication-for-third-party-tools.md) for more details. This method is only available for the United States region.
