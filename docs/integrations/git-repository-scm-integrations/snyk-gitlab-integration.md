...
Typically, the first user in a Snyk Organization, a [Snyk admin](../../snyk-admin/introduction-to-snyk-administration.md#member-user-types)/GitLab Owner or Maintainer, sets up an integration with a **GitLab Personal Access Token** or **Group Access Token.** This token is then authenticated with GitLab, enabling access by Snyk to the repositories in that GitLab account.
