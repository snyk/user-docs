...
4. Complete the fields in the configuration. Find full details about the parameters in the [Snyk GitHub repo](https://github.com/snyk/snyk-azure-pipelines-task#task-parameters) or in the next section of this doc, [Snyk Security Scan task parameters and values](./). **Note:** If you check the **Fail build if Snyk finds issue** option, then if the build fails, the pipeline job is failed by the Snyk task. If you uncheck the **Fail build if Snyk finds issue** option, the Snyk task tests for vulnerabilities, but does not cause the pipeline job to fail. When **testing a container image**, you can specify the path to the Dockerfile with the dockerfilePath property in order to receive additional information about issues in your base image. To add your Dockerfile for additional base image data when testing your container, ensure the image has been built.
