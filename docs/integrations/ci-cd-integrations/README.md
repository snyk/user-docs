...
The `snyk test` command is synchronous; it ends with an exit code. Your build system can use exit codes to either pass or fail the build based on the test results. See the [CLI reference](../../snyk-cli/cli-reference.md) for the meaning of the exit codes.
