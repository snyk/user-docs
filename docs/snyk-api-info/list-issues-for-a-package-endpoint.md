...
The current release supports the following Package URL types: `maven`, `npm`, `cocoapods`, `composer`, `gem`, `nuget`, `pypi`, `hex`, `swift`, `cargo`, `apk`, `deb` and `rpm.` If you are interested in support for additional ecosystems, submit a request to [Snyk Support](https://support.snyk.io/hc/en-us/requests/new).
