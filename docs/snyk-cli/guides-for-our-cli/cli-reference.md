...
Default: 4 \(the current working directory and 3 sub-directories\)`--exclude`=DIRECTORY\[,DIRECTORY\]...&gt;\(only in `test` and `monitor` commands\) Can be used with --all-projects and --yarn-workspaces to indicate sub-directories to exclude. Directories must be comma-separated.
...
Please note that currently the following flags are not supported:`--fail-on`=all\|upgradable\|patchable
...
[How to use Service Accounts](https://snyk.co/ucT6L)`SNYK_API`Sets API host to use for Snyk requests. Useful for on-premise instances and configuring proxies.`SNYK_CFG_`Allows you to override any key that's also available as `snyk config` option.
