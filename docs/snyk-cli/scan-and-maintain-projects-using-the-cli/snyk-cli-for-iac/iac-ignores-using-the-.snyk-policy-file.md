...
When you scan IaC configuration files using the Snyk CLI `iac test` command, you can ignore issues that are not relevant to you by using the [`.snyk` policy file](../../../scan-with-snyk/the-.snyk-file.md) **for current IaC only**. Snyk recommends that you store and version the `.snyk` file in the root of the working directory where you store your IaC configuration files. This file can be created with the `snyk ignore` command. For details see [Ignore vulnerabilities using Snyk CLI](../ignore-vulnerabilities-using-the-snyk-cli.md).
...
The `.snyk` file has some limitations for IaC Projects. See [The `.snyk` file](../../../scan-with-snyk/the-.snyk-file.md) for standard functionality.
