...
The Snyk CLI is a build-time tool to find and fix known vulnerabilities in your projects. For a more detailed description of the Snyk CLI and Snyk, see [Snyk CLI](./). For an introduction on how to use the Snyk CLI, see [Getting started with the CLI](getting-started-with-the-cli.md).
...
Each command in this list is linked to the corresponding help page in these docs. A list of all the [options](cli-reference.md#options) for Snyk CLI commands is at the end of this page. The options are explained in detail in the help for each command.
