...
* **Organization API key:** View or copy the Organization API key. [Service accounts](../../enterprise-configurations/service-accounts/) require an API key (token) to substitute for standard user credentials. Click the **Manage Service accounts** button to set up details for service accounts.
