...
* **Organization API key:** View or copy the Organization API key. [Service accounts](../../../getting-started-with-the-snyk-enterprise-plan/service-accounts/) require an API key (token) to substitute for standard user credentials. Click the **Manage Service accounts** button to set up details for service accounts.
