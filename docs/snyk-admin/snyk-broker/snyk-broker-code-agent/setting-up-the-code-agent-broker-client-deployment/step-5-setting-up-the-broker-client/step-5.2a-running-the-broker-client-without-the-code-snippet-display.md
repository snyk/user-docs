...
* `-e <SCM_domain>=` is your SCM domain name, without http/https, for example, `snyk.git.com`. For each SCM. Use the parameter for your SCM:
  ...
  * **GitHub Enterprise**: `-e GITHUB`\
    For [GitHub Enterprise](../../../install-and-configure-broker-using-docker/setup-broker-with-github-enterprise.md) add the following parameters also:\
  ...
  * **Azure Repos**: `-e AZURE_REPOS_HOST`\
    For [Azure Repos](../../../install-and-configure-broker-using-docker/setup-broker-with-azure-repos.md) add the following parameter also:\
