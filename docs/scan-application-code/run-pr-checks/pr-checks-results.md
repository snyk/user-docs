...
2. Find the conversation card showing the PR Checks results. For this example, the result is set to **Failed** and is manually changed to **Passed**. \
   \
   **Note**: Issues that have previously been ignored via the Snyk web UI in the related open source or Code Project will not be flagged in these checks. This is to reflect ignores across feature branch PRs.
