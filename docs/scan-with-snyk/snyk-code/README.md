...
| **SaaS with a self-hosted Git server (requires Snyk Broker)**        | <p>For customers with SCMs that are not publicly accessible from the internet and want to connect Snyk Code with their local self-hosted SCM. <br><br>See <a href="../../enterprise-configuration-and-implementation/snyk-broker/snyk-broker-code-agent/">Snyk Broker - Code Agent</a>.</p> |
