...
A pull request with an automatic fix for discovered vulnerabilities that Snyk can offer the user. See [Automated fix PRs](../scan-with-snyk/snyk-open-source/snyk-fix-prs-and-fix-merge-requests/automated-fix-pull-requests-for-backlog-issues-and-known-vulnerabilities.md).
