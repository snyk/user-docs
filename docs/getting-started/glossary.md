...
A demonstration of how a vulnerability can be taken advantage of. When an exploit is widely published, it is commonly referred to as an exploit "in the wild". See [View exploits](../manage-issues/prioritizing-and-managing-issues/view-exploits.md).
...
Snyk scores issues (vulnerabilities and licenses), to help prioritize treatment of each one. Scores are based on multiple factors including as the CVSS score, and range from 0 (low) to 1000 (high). See [Priority Score](../manage-issues/prioritizing-and-managing-issues/priority-score.md).
...
Whether an application contains code which will hit a vulnerable code path during execution. See [Reachable vulnerabilities](../manage-issues/prioritizing-and-managing-issues/reachable-vulnerabilities.md).
...
A severity level is applied to a vulnerability or a license issue, to indicate the risk for that item in an application. See [Severity levels](../manage-issues/prioritizing-and-managing-issues/severity-levels.md).
...
Snyk shows a Trending banner on issues that are being actively discussed on Twitter. See  [Vulnerabilities with Social Trends](../manage-issues/prioritizing-and-managing-issues/vulnerabilities-with-social-trends.md).
