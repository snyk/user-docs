...
Snyk scores issues (vulnerabilities and licenses), to help prioritize treatment of each one. Scores are based on multiple factors including as the CVSS score, and range from 0 (low) to 1000 (high). See [Priority Score](../scan-using-snyk/find-and-manage-priority-issues/priority-score.md).
...
Whether an application contains code which will hit a vulnerable code path during execution. See [Reachable vulnerabilities](../scan-using-snyk/find-and-manage-priority-issues/reachable-vulnerabilities.md).
